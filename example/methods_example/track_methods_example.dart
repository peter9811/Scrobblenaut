...
void main() async {
  ...
  // track.addTags
  print(
      'Result of addTag request: ${await scrobblenaut.track.addTags(track: 'Flay', artist: 'PLEEG', tags: ['anime'])}');

  print('######################track.getCorrection###########################');

  // track.getCorrection
  for (var track in (await scrobblenaut.track.getCorrection(track: 'TOMOYO', artist: 'Zekk'))) {
  ...
  // track.getSimilar
  (await scrobblenaut.track.getSimilar(track: 'Tomoyo', artist: 'Zekk'))?.forEach((Track track) {
  ...
  // track.getTags
  (await scrobblenaut.track.getTags(track: 'Hells Bells', artist: 'AC/DC', user: 'nebulino'))?.forEach((Tag tag) {
  ...
  // track.getTopTags
  (await scrobblenaut.track.getTopTags(track: 'Hells Bells', artist: 'AC/DC'))?.forEach((Tag tag) {
    print('Tag name: ${tag.name} | Tag url: ${tag.url}');
  });
  ...
  // track.love
  print('Result of love request: ${await scrobblenaut.track.love(track: 'Sunlight', artist: 'PLEEG')}');

  print('########################track.removeTag#############################');

  // track.removeTag
  print(
      'Result of removeTag request: ${await scrobblenaut.track.removeTag(track: 'Flay', artist: 'PLEEG', tag: 'anime')}');
  ...
  // track.search
  (await scrobblenaut.track.search(track: 'Horizon', artist: 'PLEEG')).tracks?.forEach((Track track) {
  ...
  // track.unlove
  print('Result of unlove request: ${await scrobblenaut.track.unLove(track: 'Sunlight', artist: 'PLEEG')}');

  print('#####################track.updateNowPlaying#########################');

  // track.updateNowPlaying
  final nowPlayingResponse = await scrobblenaut.track.updateNowPlaying(track: 'Beautiful Moonlight', artist: 'QU4RTZ');
  // YAY. IT WORKS!
