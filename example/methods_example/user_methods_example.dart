...
void main() async {
  ...
  // user.getLovedTracks
  (await scrobblenaut.user.getLovedTracks(user: 'nebulino'))?.forEach((Track track) {
  ...
  (await scrobblenaut.user.getPersonalTags(user: 'nebulino', tag: 'anime', taggingType: TaggingType.track))
      .tracks
      ?.forEach((Track track) {
    print('Track Name: ${track.name} | Track URL: ${track.url}');
  });
  ...
  (await scrobblenaut.user.getRecentTracks(user: 'nebulino'))?.forEach((Track track) {
    print('Track Name: ${track.name} | Track URL: ${track.url}');
  });
  ...
  // user.getTopAlbums
  (await scrobblenaut.user.getTopAlbums(user: 'nebulino'))?.forEach((Album album) {
  ...
  // user.getTopArtists
  (await scrobblenaut.user.getTopArtists(user: 'nebulino'))?.forEach((Artist artist) {
  ...
  // user.getTopTracks
  for (var track in (await scrobblenaut.user.getTopTracks(user: 'nebulino'))) {
  ...
  (await scrobblenaut.user.getWeeklyArtistChart(user: 'nebulino'))?.forEach((Artist artist) {
    print('Top Artist Name: ${artist.name} | Top Artist URL: ${artist.url}');
  });
  ...
  (await scrobblenaut.user.getWeeklyChartList(user: 'nebulino'))?.forEach((Chart chart) {
  ...
  (await scrobblenaut.user.getWeeklyTrackChart(user: 'nebulino'))?.forEach((Track track) {
